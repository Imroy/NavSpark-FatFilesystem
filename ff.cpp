...
//  for(i=0;i<512;i+=8)
//  {
//    data[0] = ptr[i]<<24|ptr[i+1]<<16|ptr[i+2]<<8|ptr[i+3];
//    data[1] = ptr[i+4]<<24|ptr[i+5]<<16|ptr[i+6]<<8|ptr[i+7];

//    len = sprintf( buf, "%08X,%08X\r\n",data[0],data[1]);
//    UART_PutLine( 0, buf, len );
//  }
//#endif
//  len = sprintf(buf, "$chk_mounted(), fmt=%d\r\n",fmt);
//  UART_PutLine( 0, buf, len );  
//#endif  
...
#ifdef SD_MMC_DEBUG
  sd_mmc_debug_s("$sd_v2", "f_open ", path);
#endif
...
#ifdef SD_MMC_DEBUG
  sd_mmc_debug_n("$sd_v2", "f_read bytes ", btr);
#endif
...
#ifdef SD_MMC_DEBUG
//  sd_mmc_debug_n("$sd_v2", "f_write bytes ", btw);
#endif
...
#ifdef SD_MMC_DEBUG
  sd_mmc_debug_s("$sd_v2", "f_opendir ", path);
#endif  
